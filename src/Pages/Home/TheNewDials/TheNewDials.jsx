...
const TheNewDials = () => {
    ...
    const textData = [
        ...
        },
        {
            ...
            detail: 'Fine grain stainless steel for the ultimate stealth wealth look.'
        },
        {
            image: group3,
            name: 'THE DIAMOND ROMAN DIAL',
            detail: 'A true classic since 900 BC'
        },
    ]

    const { isDark } = useContext(Context)
    return (
        <div className=' mid-xl:mt-28 mt-14 lg:h-screen bg-auto bg-no-repeat bg-center xl:bg-cover' style={{ backgroundImage: `url(${image})` }}>

            <div className='py-10 lg:py-16'>
                <h1 className={`lg:text-[30px] md:text-[27px] text-[24px] uppercase text-center ${isDark ? 'text-white' : 'text-black'}`}>
                    The Dials
                </h1>
                ...
                <div className='flex flex-col gap-5 lg:gap-[53px] '>
                    {
                        textData?.map((text, index) => {
                            ...
                            return (
                                <div key={index} className={`${isDark ? 'text-white' : 'text-black'}  grid lg:grid-cols-3 gap-5 mid-xl:gap-x-[65px] gap-y-10 mid-xl:gap-y-10 items-center`}>
                                    <div className='lg:col-span-2 hidden lg:block'>
                                        <img src={image} alt="" className='w-10/12 mid-xl:w-11/12  mx-auto ' />
                                    </div>
                                    <div className='lg:flex flex-col gap-3 mid-xl:gap-1 hidden'>
                                        <h1 className='text-[20px] mid-xl:text-[25px]'>{name}</h1>
                                        <p className={`leading-[23.7px] mid-xl:text-[20px] ${isDark ? 'text-[#8C8C8C]' : 'text-[#474A59]'}`}>{detail}</p>
                                    </div>
